...
# def debug_pending_pets():
#     """
#     Retorna todos os registros pendentes diretamente do banco de dados.
#     Usado para diagnóstico.
#     """
#     with connect() as conn:
#         cursor = conn.cursor()
#         cursor.execute("SELECT rowid, * FROM cadastropet WHERE aprovado = 0")
#         return cursor.fetchall()

# def debug_all_pets():
#     """
#     Retorna todos os registros do banco de dados, aprovados e pendentes.
#     Usado para diagnóstico.
#     """
#     with connect() as conn:
#         cursor = conn.cursor()
#         cursor.execute("SELECT * FROM cadastropet")
#         return cursor.fetchall()
...
# def fetch_pets(status):
#     with connect() as conn:
#         cursor = conn.cursor()
#         cursor.execute("SELECT * FROM cadastropet WHERE statuspet = ? AND aprovado = 1", (status,))
#         columns = [col[0] for col in cursor.description]
#         return [dict(zip(columns, row)) for row in cursor.fetchall()]
